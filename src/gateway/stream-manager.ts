...
export class StreamManager implements IStreamManager {
  ...
  startStream(userId: string, streamId: string, generator: AsyncGenerator<string>): void {
    const set = this.active.get(userId) ?? new Set<string>();
    set.add(streamId);
    this.active.set(userId, set);
    void (async () => { for await (const _chunk of generator) { if (!set.has(streamId)) break; } set.delete(streamId); })();
  }
