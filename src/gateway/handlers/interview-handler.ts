...
export class InterviewHandler {
  ...
  async handleStart(context: HandlerContext, payload: ClientEvents['interview:start']): Promise<void> { const session = await this.sessionManager.get(payload.resumeId as any) as InterviewSession; const firstQuestion = await this.interviewEngine.initializeInterview(session); context.ws.send(JSON.stringify({ event: 'interview:started', data: { sessionId: session.id, stage: session.stage, firstQuestion, config: session.config }, timestamp: new Date().toISOString(), userId: context.userId })); }
